<div class="overflow-x-auto">
                    ...
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($empaque, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('empaques-show')
                                    <a href="{{ route('empaques.show', $empaque) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('empaques-edit')
                                    <a href="{{ route('empaques.edit', $empaque) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('empaques-destroy')
                                    <button wire:click="confirmarEliminar({{ $empaque->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
    ...
    <div class="mt-3 flex items-center justify-between">
        ...
        <div>
            {{ $empaques->links('vendor.pagination.tailwind') }}
        </div>
    </div>
