<div class="overflow-x-auto">
                    ...
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($unidad, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('unidades-show')
                                    <a href="{{ route('unidades.show', $unidad) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('unidades-edit')
                                    <a href="{{ route('unidades.edit', $unidad) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('unidades-destroy')
                                    <button wire:click="confirmarEliminar({{ $unidad->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
    ...
    <div class="mt-3 flex items-center justify-between">
        ...
        <div>
            {{ $unidades->links('vendor.pagination.tailwind') }}
        </div>
    </div>
