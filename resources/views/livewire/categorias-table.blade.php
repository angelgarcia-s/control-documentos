                    ...
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($categoria, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('categorias-show')
                                    <a href="{{ route('categorias.show', $categoria) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('categorias-edit')
                                    <a href="{{ route('categorias.edit', $categoria) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('categorias-destroy')
                                    <button wire:click="confirmarEliminar({{ $categoria->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
