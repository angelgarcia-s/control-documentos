                    ...
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($codigoBarra, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('codigos-barras-show')
                                    <a href="{{ route('codigos-barras.show', $codigoBarra) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('codigos-barras-edit')
                                    <a href="{{ route('codigos-barras.edit', $codigoBarra) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('codigos-barras-destroy')
                                    <button wire:click="confirmarEliminar({{ $codigoBarra->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
