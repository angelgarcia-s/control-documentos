<div class="overflow-x-auto">
                    ...
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($tamano, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('tamanos-show')
                                    <a href="{{ route('tamanos.show', $tamano) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('tamanos-edit')
                                    <a href="{{ route('tamanos.edit', $tamano) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('tamanos-destroy')
                                    <button wire:click="confirmarEliminar({{ $tamano->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
    ...
    <div class="mt-3 flex items-center justify-between">
        ...
        <div>
            {{ $tamanos->links('vendor.pagination.tailwind') }}
        </div>
    </div>
