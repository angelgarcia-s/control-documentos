<div class="overflow-x-auto">
    <div class="ti-custom-table ti-striped-table ti-custom-table-hover">
        ...
        <table class="w-full bg-white table-auto whitespace-nowrap border border-gray-300 rounded-lg">
            ...
            <tbody>
                @foreach($barnices as $barniz)
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($barniz, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('barnices-show')
                                    <a href="{{ route('barnices.show', $barniz) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('barnices-edit')
                                    <a href="{{ route('barnices.edit', $barniz) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('barnices-destroy')
                                    <button wire:click="confirmarEliminar({{ $barniz->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
    ...
    <div class="mt-3 flex items-center justify-between">
        ...
        <div>
            {{ $barnices->links('vendor.pagination.tailwind') }}
        </div>
    </div>
