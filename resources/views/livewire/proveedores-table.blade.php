                    ...
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($proveedor, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('proveedores-show')
                                    <a href="{{ route('proveedores.show', $proveedor) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('proveedores-edit')
                                    <a href="{{ route('proveedores.edit', $proveedor) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('proveedores-destroy')
                                    <button wire:click="confirmarEliminar({{ $proveedor->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
