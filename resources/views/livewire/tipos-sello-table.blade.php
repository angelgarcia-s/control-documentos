<div class="overflow-x-auto">
    <div class="ti-custom-table ti-striped-table ti-custom-table-hover">
        ...
        <table class="w-full bg-white table-auto whitespace-nowrap border border-gray-300 rounded-lg">
            ...
            <tbody>
                @foreach($tiposSello as $tipoSello)
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($tipoSello, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('tipos-sello-show')
                                    <a href="{{ route('tipos-sello.show', $tipoSello) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('tipos-sello-edit')
                                    <a href="{{ route('tipos-sello.edit', $tipoSello) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('tipos-sello-destroy')
                                    <button wire:click="confirmarEliminar({{ $tipoSello->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
    ...
    <div class="mt-3 flex items-center justify-between">
        ...
        <div>
            {{ $tiposSello->links('vendor.pagination.tailwind') }}
        </div>
    </div>
