                    ...
                    <tr class="border-b hover:bg-gray-100 dark:hover:bg-gray-800">
                        @foreach($columnas as $columna)
                            <td class="py-3 px-6 border">
                                {{ $this->getColumnValue($user, $columna) }}
                            </td>
                        @endforeach
                        <td class="py-3 px-6 border">
                            <div class="flex items-center space-x-2">
                                @can('usuarios-show')
                                    <a href="{{ route('usuarios.show', $user) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-eye-line"></i>
                                    </a>
                                @endcan
                                @can('usuarios-edit')
                                    <a href="{{ route('usuarios.edit', $user) }}" class="ti-btn text-lg text-slate-400 !py-1 !px-1 ti-btn-wave">
                                        <i class="ri-pencil-line"></i>
                                    </a>
                                @endcan
                                @can('usuarios-destroy')
                                    <button wire:click="confirmarEliminar({{ $user->id }})" class="ti-btn text-lg text-rose-400 !py-1 !px-1 ti-btn-wave">
