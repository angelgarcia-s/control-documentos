    ...
    plugins: [
        ...
        }),
        {
            ...
            writeBundle: async () => {
                ...
                // Combinar devDependencies y dependencies
                const allDeps = {
                ...
                for (const dep of Object.keys(allDeps)) {
                    ...
                    let destPath = join(destDir, dep);
                    console.log(`Intentando copiar ${srcPath} a ${destPath}`);
                    if (await fsExtra.pathExists(srcPath)) {
                        await fsExtra.copy(srcPath, destPath, { overwrite: true, recursive: true });
                        console.log(`Copiado ${dep} desde dist`);
                        // No eliminamos 'dist' aquí para mantener la estructura original
                    } else {
                        // Casos especiales como @popperjs/core que usa 'dist/umd'
                        srcPath = join('node_modules', dep, 'dist/umd');
                        destPath = join(destDir, dep, 'umd');
                        console.log(`Intentando copiar ${srcPath} a ${destPath} (caso especial)`);
                        if (await fsExtra.pathExists(srcPath)) {
                            await fsExtra.copy(srcPath, destPath, { overwrite: true, recursive: true });
                            console.log(`Copiado ${dep} desde dist/umd`);
                        } else {
                            // Si no hay 'dist' ni 'dist/umd', copia toda la carpeta de la dependencia
                            srcPath = join('node_modules', dep);
                            destPath = join(destDir, dep);
                            console.log(`Intentando copiar ${srcPath} a ${destPath} (fallback)`);
                            if (await fsExtra.pathExists(srcPath)) {
                                await fsExtra.copy(srcPath, destPath, { overwrite: true, recursive: true });
                                console.log(`Copiado ${dep} completo`);
                            } else {
                                console.log(`No se encontró ${srcPath} para ${dep}`);
                            }
                        }
                    }
                }
